...
using TeamMoiraiActions.Contexts;
using TeamMoiraiActions.Utils;
using Octokit;
using System.Text.Json;

namespace TeamMoiraiActions.Commands;
...
public class GitCommand()
{
    ...
    public async Task<(bool commited, string sha, string branchName, string isBranchCreated)> CommitWithSignAsync(bool dryRun, string tag, string[] modifiedPaths)
    {
        ...
        catch (ProcessErrorException)
        {
            ...
            var client = new GitHubClient(new ProductHeaderValue("TeamMoiraiActions"))
            {
